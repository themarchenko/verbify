...
function PixelBackground({
  ...
  const pixelsRef = React.useRef<PixelUnit[]>([])
  const animationRef = React.useRef<ReturnType<typeof requestAnimationFrame> | null>(null)
  ...
  const initPixels = React.useCallback(() => {
    ...
    for (let x = 0; x < width; x += gapInt) {
      for (let y = 0; y < height; y += gapInt) {
        ...
        const delay = reducedMotionRef.current ? 0 : calculateDelay(pattern, x, y, width, height)
        pxs.push(
          new PixelUnit(
            canvasRef.current!,
            ctx,
  ...
  const doAnimate = React.useCallback(function animate(fnName: 'appear' | 'disappear') {
  ...
  const onMouseLeave = React.useCallback(() => handleAnimation('disappear'), [handleAnimation])
